...
(  
    ...
    try { 
        ...
        if ($fontFile.Extension -in ('.otf','.ttf'))
        {
            #get font name
        ...
        #[System.Runtime.Interopservices.Marshal]::ReleaseComObject($fontFile) | out-null 
        #Remove-Variable fontFile               
...
foreach ($FontItem in (Get-ChildItem -Path $sFontsFolder | 
Where-Object {($_.Name -like '*.ttf') -or ($_.Name -like '*.otf') -or ($_.Name -like '*.fon') -or ($_.Name -like '*.ttc') })) {  
